...
export function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.data?.errors === "'location_id'") {
      ...
      } else if (error.response?.data?.errors?.business_profile) {
        const businessError = error.response?.data?.errors?.business_profile;
        toast.error("Registration Failed", {
        ...
        const generalError = error.response?.data?.errors?.error;
